...
; New variables for elimination style
player_alive BYTE 1,1,1,1  ; 1=alive, 0=eliminated
target_choice BYTE 0
elimination_msg BYTE "*** PLAYER ",0
eliminated_suffix BYTE " ELIMINATED! ***",0Dh,0Ah,0
...
how_to_play6 BYTE ">Players can attack ANY living opponent",0Dh,0Ah,0
how_to_play7 BYTE ">Eliminated players skip turns",0Dh,0Ah,0
how_to_play8 BYTE ">Enter coordinates like 'A5' to attack",0Dh,0Ah,0
how_to_play9 BYTE ">'X'=Hit'O'=Miss,'.'=Unknown",0Dh,0Ah,0
how_to_play10 BYTE ">Last player with ships remaining WINS!",0Dh,0Ah,0Dh,0Ah,0
...
credits6 BYTE ">4-player elimination gameplay",0Dh,0Ah,0
credits7 BYTE ">Strategic target selection",0Dh,0Ah,0
credits8 BYTE ">Random ship placement",0Dh,0Ah,0
credits9 BYTE ">Graphical board display",0Dh,0Ah,0
...
; New messages for target selection
available_targets_msg BYTE "Available targets: ",0
choose_target_msg BYTE "Choose target player: ",0
invalid_target_msg BYTE "Invalid target! Player eliminated or yourself.",0Dh,0Ah,0
attacking_msg BYTE "Attacking Player ",0
...
; Reset player status
mov player_alive[0], 1
mov player_alive[1], 1  
mov player_alive[2], 1
mov player_alive[3], 1

; Reset ship counts
mov ships_remaining[0], 5
mov ships_remaining[1], 5
mov ships_remaining[2], 5  
mov ships_remaining[3], 5
...
;Show game state and get target choice
call ClearScreen
call DisplayGameState
call GetTargetChoice
...
GetTargetChoice PROC
InputTarget:
mov edx, OFFSET available_targets_msg
call WriteString
...
mov esi,0
DisplayTargetsLoop:
cmp player_alive[esi], 1
jne NextTarget
    
mov eax,esi
inc eax
cmp al, current_player
je NextTarget  ; Skip self
...
NextTarget:
inc esi
loop DisplayTargetsLoop
call Crlf
    
; Get target choice
mov edx, OFFSET choose_target_msg
call WriteString
call ReadInt
...
mov ebx, eax
dec ebx
cmp player_alive[ebx], 1  ; Check if alive
jne BadTarget
    
mov eax, ebx
inc eax
cmp al, current_player    ; Check not self
je BadTarget
    
mov target_player, al
    
; Show who we're attacking
mov edx, OFFSET attacking_msg
call WriteString
...
BadTarget:
mov edx, OFFSET invalid_target_msg
call WriteString
mov eax, 1000
call Delay
call ClearScreen
...
;Display ships remaining for current target
...
cmp ships_remaining[ebx], 0
jg AttackComplete
    
; Eliminate player
mov player_alive[ebx], 0
mov edx, OFFSET elimination_msg
call WriteString
mov al, target_player
add al, '0'
call WriteChar
mov edx, OFFSET eliminated_suffix
call WriteString
...
mov ecx,4
mov esi,0
mov ebx,0  ; Count alive players
mov edx, -1 ; Last alive player index
    
CheckAliveLoop:
cmp player_alive[esi], 1
jne NextCheck
inc ebx
mov edx, esi  ; Remember last alive player
...
SwitchPlayer PROC
mov ecx, 4  ; Max 4 attempts to find next alive player
...
; If no alive player found (shouldn't happen)
mov current_player, 1
PlayerFound:
ret
SwitchPlayer ENDP
